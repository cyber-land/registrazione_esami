...
const TestForm = (params) => {
  const { student, setTests, exams, server_addr, retrieveTestsOfStudent } =
    useContext(Ctx);
  ...
  return (
    <>
      ...
      <h3 className="uk-child-width-1-3@s" uk-grid="true">
        <div className="uk-panel uk-text-break uk-text-center uk-text-uppercase">
          {studente.cognome}
        </div>
        <div className="uk-panel uk-text-break uk-text-center uk-text-uppercase">
          {studente.nome}
      ...
      <form uk-grid="true">
        ...
        <div className="uk-width-1-6">
          <select
            ...
            value={esame}
            onChange={(e) => {
              setEsame(e.target.value);
            }}
          >
            {exams.map((exam, pos) => (
              <option key={pos} value={exam.id}>
                {" "}
                {exam.data.split(" ")[0]}{" "}
              </option>
            ))}
