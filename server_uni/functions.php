...
from prova, esame where esame.id = prova.id_esame and id_studente = :id_studente order by data desc limit 1';
	...
	$res = $stmt->fetchAll();
	if ($res[0]['result']) return array(false, 'data invalida (precedente all\'ultima prova eseguita)');
	...
	if ($tipologia == 'teoria' && $valutazione <= 15) {
		...
		return array(false, 'tipologia non valida (la teoria deve essere la proma prova o deve essere 
		preceduta da un altro esame di teoria)');
		...
		if (!$result || $result[0]['tipologia'] == 'orale') {
			return array(false, 'tipologia non valida (non si può fare l\'esame di teoria dopo l\'orale)');
		}
		...
		$stmt->execute(['id_studente' => $test->{'id_studente'}]);
		$result = $stmt->fetchAll();
		if (!$result) return array(false, 'tipologia non valida (la programmazione deve essere preceduta da 
		un valido esame di teoria)');
		...
		return array(false, 'tipologia non valida (dopo due tentativi falliti della seconda prova bisogna
		obbligatoriamente rifare la prima)');
		...
		return array(false, 'tipologia non valida (l\'orale deve essere obbligatoriamente precedeuto da 
		un valido esame di programmazione)');
	} else {
		return array(false, 'tipologia non valida (non esiste una tipologia con questo nome)');
	}
